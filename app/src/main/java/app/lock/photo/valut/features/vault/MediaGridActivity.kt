...
@AndroidEntryPoint
class MediaGridActivity : SecureVaultActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = FragmentMediaGridBinding.inflate(layoutInflater)
    ...
    private fun setupToolbar() = with(binding.toolbar) {
        title = this@MediaGridActivity.title
    ...
    private fun setupRecycler() {
        ...
        binding.recyclerView.layoutManager =
            GridLayoutManager(this, resources.getInteger(R.integer.grid_span))
    ...
    private fun openItem(item: VaultMediaUiModel) {
        if (item.mediaType == MediaType.VIDEO) {
            startActivity(VideoPlayerActivity.intent(this, item.id))
        } else {
            ...
            startActivity(PhotoViewerActivity.intent(this, photoIds.toLongArray(), startIndex))
        }
    }
    ...
    private fun onPickedForAlbum(uris: List<Uri>) {
        ...
        val albumId = intent.getLongExtra(MediaGridViewModel.ARG_ALBUM_ID, -1L)
        startActivity(ImportProgressActivity.intent(this, uris, albumId))
    }
    ...
    companion object {
        ...
        ): Intent = Intent(context, MediaGridActivity::class.java).apply {
            putExtra(MediaGridViewModel.ARG_SOURCE, source.name)
            putExtra(MediaGridViewModel.ARG_ALBUM_ID, albumId)
            putExtra(ARG_TITLE, title)
            putExtra(MediaGridViewModel.ARG_MEDIA_FILTER, mediaFilter?.name)
        }
    }
}
